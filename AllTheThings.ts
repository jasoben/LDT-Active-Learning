function main(workbook: ExcelScript.Workbook) {
    ...
    type room = {
        ...
        console.log(NCH107.schedule.get("M"));
    
    /*
        let sheets = workbook.getWorksheets();
    
        // Assign specific worksheets based on name
    
        for (let i = 0; i < sheets.length; i++) {
    
            if (sheets[i].contain("18")) {
                classRooms18to30 = sheets[i];
            }
    
            if (sheets[i].contain("31")) {
                classRooms31to54 = sheets[i];
            }
    
            if (sheets[i].contain("55")) {
                classRooms55to62 = sheets[i];
            }
    
            if (sheets[i].contain("63")) {
                classRooms63to99 = sheets[i];
            }
