...
function main(workbook: ExcelScript.Workbook) {
  ...
  function FillOpenSlot(uvaClass: UVAClass, row: number, foundRoomIndex: number) {
    ...
    if (uvaClass.day.includes("M")) {
      rooms[foundRoomIndex].schedule.get("M")[row] = courseInfo;
    }
    if (uvaClass.day == "TuTh" || uvaClass.day == "T") {
      rooms[foundRoomIndex].schedule.get("T")[row] = courseInfo;
    }
    if (uvaClass.day.includes("W")) {
      rooms[foundRoomIndex].schedule.get("W")[row] = courseInfo;
    }
    if (uvaClass.day.includes("Th")) {
      rooms[foundRoomIndex].schedule.get("Th")[row] = courseInfo;
    }
    if (uvaClass.day.includes("F")) {
      rooms[foundRoomIndex].schedule.get("F")[row] = courseInfo;
    }
